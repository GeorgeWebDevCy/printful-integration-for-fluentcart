...
<div class="pifc-card">
    ...
    <p><?php esc_html_e('Enable the Printful shipping services you want to offer at checkout. For each enabled service a FluentCart shipping method is created (or updated). The actual rate is fetched live from Printful at checkout — the amount shown in FluentCart\'s shipping zone settings is a placeholder only.', 'printful-for-fluentcart'); ?></p>
    ...
    <table class="form-table pifc-services-table" role="presentation">
        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><input type="checkbox" class="pifc-svc-enabled" data-code="<?php echo esc_attr($code); ?>" <?php checked($enabled); ?>></td>
                    <td><code><?php echo esc_html($code); ?></code></td>
                    <td><input type="text" class="pifc-svc-title regular-text" data-code="<?php echo esc_attr($code); ?>" value="<?php echo esc_attr($title); ?>"></td>
                    <td>
                        <select class="pifc-svc-zone" data-code="<?php echo esc_attr($code); ?>">
                            ...
                            <?php foreach ($zones as $zone) : ?>
                                <option value="<?php echo (int) $zone['id']; ?>" <?php selected($zoneId, (int) $zone['id']); ?>>
    ...
    <p class="submit">
        <button type="button" id="pifc-save-shipping" class="button button-primary"><?php esc_html_e('Save & Sync to FluentCart', 'printful-for-fluentcart'); ?></button>
...
<script>
window.pifcAdminInitPanel && window.pifcAdminInitPanel('shipping');
</script>
