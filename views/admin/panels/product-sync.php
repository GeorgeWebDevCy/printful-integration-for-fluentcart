...
<div class="pifc-card">
    ...
    <p><?php esc_html_e('Enter the Printful sync-product ID to import or refresh a single product. You can find the ID in your Printful store product list URL.', 'printful-for-fluentcart'); ?></p>
    <p>
        <input type="number" id="pifc-single-product-id" class="small-text" min="1" placeholder="<?php esc_attr_e('e.g. 123456789', 'printful-for-fluentcart'); ?>" <?php echo empty($settings['api_key']) ? 'disabled' : ''; ?>>
        <button type="button" id="pifc-sync-single" class="button button-secondary" <?php echo empty($settings['api_key']) ? 'disabled' : ''; ?>><?php esc_html_e('Sync Product', 'printful-for-fluentcart'); ?></button>
...
<script>
window.pifcAdminInitPanel && window.pifcAdminInitPanel('sync');
</script>
